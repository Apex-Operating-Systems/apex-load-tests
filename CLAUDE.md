...
## MERGE-TO-MAIN RULE — MANDATORY (WE ALREADY GOT BURNED BY THIS)

> On 2026-02-22, approved feature branches in apex-system were never merged to `main`.
> `main` fell 4,700 lines behind production. A later session applied work to the wrong version.
> **Hours lost. This already happened. It CANNOT happen again.**

1. After a fix/feature is approved → **merge to `main` IMMEDIATELY** and push.
2. After merging → **re-test on `main`** to confirm nothing broke.
3. Before starting new work → **check for unmerged branches** (`git branch -r --no-merged main`).
4. NEVER start new work from a feature branch. Always branch from up-to-date `main`.

---

## GITHUB CLOUD-FIRST — NEVER WORK LOCALLY

> Work done locally without pushing has caused lost changes and blocked collaboration
> between Ben and Vinny. **GitHub is the ONLY source of truth.**

1. Every edit → commit → push. Immediately. Unpushed work doesn't exist.
2. Plans, docs, checklists → committed as `.md` files on GitHub. Not saved locally.
3. When starting a session → `git fetch origin && git pull` before touching anything.
4. Ben and Vinny both work through Claude Code. GitHub is the shared workspace.
